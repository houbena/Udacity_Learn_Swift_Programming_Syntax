...
func returnLastLetter(var myString: String) -> String{
    while myString.characters.count > 1{
...
func combineLastCharacters(stringArray: [String])-> String{
    ...
    for aString in stringArray{
        rString = rString + returnLastLetter(aString)
    }
    return rString
}
...
func stringToBool(myString: String) -> Bool{    
    for c in myString.unicodeScalars{
...
func removeDirtyWords(myArray: [String])->[String]{
    var cleanArray = [String]()
    for myString in myArray{
        if myString.characters.count != 4{
            cleanArray.append(myString)
        }
    }
...
var movies:Dictionary<String,String> = [ "Boyhood":"Richard Linklater","Inception":"Christopher Nolan", "The Hurt Locker":"Ava Du Vernay", "Selma":"Ava Du Vernay", "Interstellar":"Christopher Nolan"]

class MovieArchive {
   func filterByDirector(myDic: Dictionary<String, String>, myDirector: String)-> [String]{
        var movieArray = [String]()
        
        for (movie, movieDirector) in myDic {
            if movieDirector == myDirector {
...
let archive = MovieArchive()
archive.filterByDirector(movies, myDirector: "Ava Du Vernay")
