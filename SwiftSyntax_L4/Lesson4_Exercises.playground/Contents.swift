...
for amount in oldMountainViewRent{
    newMountainViewRent.append(amount * 1.2)
}
//: ### Exercise 3
...
for (name, number) in rapperPhoneNumbers{
    newNumber = number.stringByReplacingOccurrencesOfString("(415)", withString: "(510)")
    rapperPhoneNumbers.updateValue(newNumber, forKey: name)
}
//: ## Switch Statements
